...
void set_oldfriends(vector <string> &name, vector < vector <int> > &friends, int M0, int M1) {
	...
	for (int i=0; i<N; i++) {
		...
		for(ii = doknow.begin(); ii != doknow.end(); ii++){
			...
			friends[i].push_back(*ii);
			friends[*ii].push_back(i);

			sort(friends[i].begin(), friends[i].end());
			friends[i].erase(unique(friends[i].begin(), friends[i].end()), friends[i].end());
			sort(friends[*ii].begin(), friends[*ii].end());
			friends[*ii].erase(unique(friends[*ii].begin(), friends[*ii].end()), friends[*ii].end());
		}
	}	
}
			...
			//if( i != *j ){
				for (k = friends[*j].begin(); k != friends[*j].end(); ++k) {
					...
					//new_friends[i][k] = new_friends[k][i] = 1;
						...
						sort(new_friends[*k].begin(), new_friends[*k].end());
						new_friends[*k].erase(unique(new_friends[*k].begin(), new_friends[*k].end()), new_friends[*k].end());
